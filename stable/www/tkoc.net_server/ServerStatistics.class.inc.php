...
class ServerStatistics
{
	...
	function DoTick()
	{
		...
		$this->Database->query("SELECT count(ID) as TotalHeroes FROM Hero where Status='Player' AND HeroAge='".$GLOBALS['config']['HeroAge']."'");
		$a = $this->Database->fetchArray();
		$TotalHeroes = $a['TotalHeroes'];
		...
		$this->Database->query("INSERT INTO Log (Age,Provinces,UsersActive,UsersTotal,Logins,TickTime,Tick,ServerStatus,Heroes ) VALUES
								('".$this->Config['age']."','$TotalProvinces','$ActiveUsers','$TotalUsers','$Logins',NOW(),
								 '".$this->Config['ticks']."','".$this->Config['status']."','$TotalHeroes')");
	}
}
