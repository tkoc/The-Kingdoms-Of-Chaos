...
//		$this->VACATION_MODE_MAX = $this->config['vacationMax'];
		if (($this->config['ticks'] + 50) > $this->config['AgeLength'])
		  $this->OVERPOPULATED_KILL_UNITS *= 2;
	}
	...
	function doTick () {
		...
		$config = $this->database->fetchArray();
	...
	function handleDead() {
		...
		//			print_r($prov);
					...
					// users that never logged in to province
					if (is_null($prov['days']) && $prov['aliveTicks'] > 24)
					{
						$cleanup=true;
						$this->database->query("UPDATE User set pID=-1 WHERE userID=$prov[userID]");
						$this->removeProvince($prov['pID']);
				...
				if (is_null($prov['days'])) {
					...
					$this->database->query("UPDATE Province set kiID=ABS(kiID) where pID='$prov[pID]'");
					$this->mywriteLog ($this->logFile,"$prov[pID] is no longer inactive!\n" );
				} 
			}
		}
		$this->fixKingdoms();
...
/*		$this->database->query("SELECT peasants from Province where peasants<0")
		...
		if ($this->database->numRows()>0) {
			$this->mywriteLog($this->errorLogFile,"NEGATIVE PEASANTS!  fixing peasants to 0.\n");
