...
//*	- Rewrite 31.07.2004
//************************************************
// TODO:  mywriteLog function, writeErr function
...
/* Function for auto ending */
if ($config['ticks'] >= $config['AgeLength'] && $config['status'] == 'Running')
	 $database->query("UPDATE Config set status='Ended'");	
...
'
tick: '. $config['ticks'] . '
   Script execution time: ' . (mymyclock() - $GLOBALS['game_start_clock']) . '
   STATISTICS
...
'
tick: '. $config['ticks'] . '
   Script execution time: ' . (myclock() - $GLOBALS['game_start_clock']) . '
   STATISTICS
       Number of queries   : ' . $GLOBALS['database_queries_count'] . '
       Number of fetches   : ' . $GLOBALS['database_queries_fetch_count'] . '
       Provinces           : ' . $totalUsers['totalUsers'] .'
