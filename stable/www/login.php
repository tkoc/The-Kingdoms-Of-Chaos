...
<div class="content" style="background-image:url('./img/loginback-dark.jpg'); padding:10px 0px;">
	...
	if (isset($config['status'])) {
		if ($config['status']=='Running')
			echo "Age #".$config['age'].": <b>".$config['status']."</b><br />";
		else if ($config['status']=='Pause') {
			$totalSeconds = $config['totalTickTime']*$config['pause'];
			$lastTick = strtotime($config['lastTickTime']);
			$estimatedDate = date ("l j F, G:i",$lastTick+$totalSeconds);
			echo "Age #".$config['age'].": <b>".$config['status']."d</b>. The Age will start in ".$config['pause']." ticks - $estimatedDate.<br />";
		}
		else {
			$totalSeconds = $config['totalTickTime']*$config['pause'];
			$lastTick = strtotime($config['lastTickTime']);
			$estimatedDate = date ("l j F, G:i",$lastTick+$totalSeconds);
			echo "Age #".$config['age'].": <b>".$config['status']."</b>. Next Age will open for signups in ".$config['pause']." ticks - $estimatedDate.<br />";
		}
