...
class Magic {
	...
	var $newWizards = NULL;			// Added by soptep to fix the wizards after losing a spell
	...
	function pageHandler() {
		...
		if( isset( $_GET['targetpID'] ) ) {
			...
			if( isset( $_GET['kiID'] ) )  // This if condition was put here to fix the bug when changing kingdom to be your province still selected
				$this->targetpID = $this->getProvince( $this->kiID );
			else
				$this->targetpID = $this->pID; // Changed to fix the selected yourself bug, not being you
		}
	...
	function setSpells() {
		...
		if( $this->db->query( $selectSQL ) && $this->db->numRows() ) {
			...
			while( ($className = $this->db->fetchArray())) {
				...
				if( !strcmp( $className['className'], "CleansingSpell" ) ) {
					$spellID = $id;
				}
			}
		}
		if( $spellID ) {
			$this->db->query( "DELETE FROM Spells WHERE sID='".$spellID."'" );
			$this->db->query( "DELETE FROM SpellT WHERE sID='".$spellID."'" );
		}
	}
	...
	function doTick () {
		...
		// Changed so that elves (raceid=7) can have maximum 110 mana, Soptep - 13/10/2009
		$updateSQL = "UPDATE Province set mana=LEAST( ( mana+$this->MANA_MODIFIER ), 110) WHERE spID=7";
		$this->db->query( $updateSQL );
		
		$updateSQL = "UPDATE Province set mana=LEAST( ( mana+$this->MANA_MODIFIER ), 100) WHERE spID!=7";
		$this->db->query( $updateSQL );
		
		echo "decreasing tick for spells\n";
		$updateSQL = "UPDATE Spells set ticks=( ticks-1 ) WHERE type=1";				// count down ticks
		$this->db->query( $updateSQL );
		echo "deleting done spells\n";
		...
		if( !$result['error'] ) {
			if( !$this->hasResources( $this->targetProvinceObj->acres ) ) {
				...
				$this->messageToUser .= "<br>".$this->provinceObj->getAdvisorName().", it would do no good to 
									...
									numbers, but perhaps it's better to ask them again tomorrow?";
				$result['error'] = true;
			...
			if ($this->provinceObj->OnMagicAction() == false)
			{
				if ($this->provinceObj->isOverPopulated() == true)
				{
					$this->messageToUser .=	"<br><br><b>".$this->provinceObj->getAdvisorName().", the peasants are rioting due to overpopulation.
										Under these conditions wizards are extremely weak. However they will obey at your commands!";				
				}
				else
				{
	...
	function spellCastFailure( $wizards, $targetWizards, $subMana ) {
		...
		if( $randomLoose == 1 ) {
			...
			$this->newWizards = $this->wizards - $lost; // Added by soptep to fix the wrong wizards shown after failing a spell
				...
				if( $targetWizNum ) {	//if the opposition has any wizards
					if( $this->spells[$this->sID]->baseFunction( $GLOBALS['magicConstants']->GET_CAST_ON ) == 0 ) {
						$this->targetProvinceObj->postNews( $this->targetProvinceObj->getAdvisorName().
															...
															are	working to find out which spell it was. Perhaps You should 
				...
				if( $targetWizNum ) {	//if the opposition has any wizards
					if( $this->spells[$this->sID]->baseFunction( $GLOBALS['magicConstants']->GET_CAST_ON ) == 0 ) {
						...
						$this->targetProvinceObj->postNews( $this->targetProvinceObj->getAdvisorName().
															...
															working to find out which spell it was. Perhaps You should 
															go to Your magic advisor and see what they've found?" );
					}
				} else {
					if( $this->spells[$this->sID]->castOn == 0 ) {
...
/////////////////////////////////////////////////////////////////////////////////////////
	...
	function displaySpell() {
		...
		$manaNeeded = $this->getSubMana($recWiz); // Added to show the mana cost
		...
		$html .= "</i>. They also say they recommend using no less than $recWiz $this->wizardName requiring $manaNeeded% mana for this task and that you will be able to cast the spell with $strength% strength. Keep in mind that mana cost will increase if you use less than the recommended number of wizards.";
	...
	function displayUseMagic() {
		...
		// Added by soptep to fix the wrong wizards shown after failing a spell - 22/02/2010
		if (!empty($this->newWizards) && $this->newWizards < $this->wizards) 
		...
		if ($recWiz >= $availableWizards) {
			$recWiz = $availableWizards;
		}
		
		$html = "";
		$html .= "<form action='".$_SERVER['PHP_SELF']."?".$_SERVER['QUERY_STRING']."' method='POST' name='useMagic'>";
		$html .= $GLOBALS['fcid_post'];
		$html .= 	"Use <input type='textfield' name='wizards' class='form' value='$recWiz' maxlength='7'
	...
	function displayActiveSpells() {
		...
		if( strlen( $ownSpellsHtml ) ) {
			...
			$ownSpellsHtml =	$this->provinceObj->getAdvisorName().
								...
								attack our enemies right away, but perhaps You have some other plans?";
		}
	...
	function chooseProvinceSpell() {
		$html = '<script type="text/javascript">
			function redirect (pid, kiid) {
				var command = "location.href = \'./magic.php?targetpID="+pid+"&kiID="+kiid+"\'";
		...
		$html .= $GLOBALS['fcid_post'];
		$html .= "
					<select name='targetpID' class='form' onchange='redirect(this.value, kiID.value)'>";
		...
		$html .= 	"</select>
					<input id='kiID' type='hidden' name='kiID' value='".$this->kiID."'>
	...
	function getDisplay() {
		...
		$result .= "<br><table width='100%' cols='2' align='center' border='0' cellpadding='2' cellspacing=1'>";
		$result .= "<tr><td nowrap>
