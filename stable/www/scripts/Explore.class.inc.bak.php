...
//die();
	...
	class Explore {
		...
		function writeExplore() {
			...
			$information = "<center>This is where you send your ".$milObj->getName()." to explore land. Each and everyone of your ".$milObj->getName()." is capable
			<br>of exploring <b><i>about</i></b> ".round($this->getLandPrSoldier(),2)." acres. In other words, you need <b><i>about</i></b> ".round($this->getSoldiersPrAcre(), 0)." 
			...
			<br>the new lands, and desert your army<br><br><b><i>Just a final word... you'll probably not be able to explore more than<br> ".$this->MAXLAND."% of your own acre amount, so i will not advice to send any more than $this->maMilS ".$milObj->getName()."</i></b></center>";

			$retVal .= "<br><center><h1>Exploring</h1></center><br><br>$information";

			$table = "\n<br><center>\n<form class='form' name='explForm' action='".$this->self()."' method='post'><table class='buildingsTable' width='80%'>\n\t<tr>";
	...
	document.explForm.totCost.value=(document.explForm.num.value * $this->cost);'></center>";
		...
		function handleExplore() {
			...
			if($_POST['num'])  $this->num = $num = $_POST['num']; 
			else $this->num = $num = 0;
			if(isset($_POST['exploreOK']) && is_numeric($this->num) && $this->num > 0) {
			 //   echo "<br>debug handleExplore: SubmitButton is pushed, and the amount to send to training is numeric";
				...
				if(($cost <= $this->gold) &&  ($this->num <= $this->soldiers)) {
					...
					$this->milObj->killUnits($this->militaryConstants->SOLDIERS, $num, false);
					$this->pObj->useResource($cost, 0, 0);
					$this->exploreLand();
					if($this->num > $this->maMilS) {
						$retVal .= "<center><br><br>I see you didn't follow my advice... so now a good deal of the ".$milObj->getName()." you sent where to no use. I wish I cold tell you what happend to them, but I really don't know. There are rumors about a big red dragon lurking in the mountains.. but I really don't know</center>";
					}
...
//			$milObj = $mil['object'];
			...
			$retVal = "\n<center><table class='buildingsTable' width='80%'>\n\t<tr>";
		...
		function doTick() {
			$sqlCountDown = "update ProgressExpl set tick=tick-1 where tick>0";
			$sqlUpdate = "update Province, ProgressExpl set Province.acres=Province.acres+ProgressExpl.num_acers where Province.pID=ProgressExpl.pID AND ProgressExpl.tick<=0";
		...
		function run() {
			$retVal = "";
			$retVal .= $this->handleExplore();
			$retVal .= $this->writeExplore();
			$retVal .= "<br><br><center><h1>Land currently being explored</h1></center>".$this->showExplorProg();
			return $retVal;
		}
