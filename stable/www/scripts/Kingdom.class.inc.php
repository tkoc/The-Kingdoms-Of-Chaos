...
$html .= '
	...
	function showKingdom ($kingdomID=false) {
		...
		$body .= '<table align="center" width="70%" nowrap cellspacing="2">
		   				<tr>
		...
		$body .= '<tr>
						<td align="left" width="40%" nowrap class="rep3">
							<b>Name: </b>
						</td>
						<td align="right" width="10%" class="rep3">
							<b>Race:</b>
						</td>
						<td align="right" width="10%" class="rep3">
							<b>Acres:</b>
						</td>
						<td align="left"  width="20%" class="rep3">
							<b>Networth:</b>
						</td>
...
//			print_r($province);
				...
				$body .= "<a href=\"provinceAction.php".$GLOBALS['fcid']."?victim=".$province->pID."\" class='rep'>". $province->provinceName;
				if ($province->isKing()) 
					$body .="&nbsp;<b>(&nbsp;".$province->getShortTitle()."&nbsp;)</b>";
				if ($province->isProtected() && $province->vacationmode==false) 
					$body .="&nbsp;<i>( protected )</i>";
				else if ($province->isProtected() && $province->vacationmode==true) 
				{
					$body .="&nbsp;<i>( vacation )</i>";
				}
				$body .= "</a>";
				...
				if (!$province->isAlive()) {
					$row['acres']='Deleted'; $row['networth'] = 'Deleted';
				} else {
					$row['acres']=$province->acres;
					$row['networth']=$province->networth;
				}
				$body .= "</td><td align='right' class='rep1'>" .$row['speciesName'] ."<td align='right' class='rep1'>" .writeChaosNumber($row["acres"]) . "</td>
					<td align='right' class='rep1'>" .writeChaosNumber($row["networth"]) ."</td>
					<td align='right' class='rep1'>".(($uglyhack) ? "N/A" : $province->getThieveryRank())."</td>
<td align='right' class='rep1'>".(($uglyhack) ? "N/A" : $province->getMagicRank())."</td>					
</tr>";
			}
