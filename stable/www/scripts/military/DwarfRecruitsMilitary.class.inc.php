...
require_once("MilitaryBase.class.inc.php");
	...
	if( !class_exists("DwarfRecruitsMilitary")) {
		class DwarfRecruitsMilitary extends MilitaryBase {
			var $picture = "recruite";
			var $MilitaryConst = NULL;
			
			function DwarfRecruitsMilitary( $militaryID ) {
				$this->MilitaryConst = $GLOBALS['MilitaryConst'];
				$info = "Newly educated military. These soldiers should be used for training better military or exploring new land. Thieves and Wizards also need these basic skills of fight. You'll need 12 days to train this unit";
				$type = $this->MilitaryConst->SOLDIERS;
				$ticks = 12;
				$cost = array("gold"=>300, "metal"=>0, "food"=>0);
				$strength = array("attack"=>1, "defense"=>2);
				$this->MilitaryBase($militaryID, "Warriors", $info ,$type, $ticks, $cost, $strength);
