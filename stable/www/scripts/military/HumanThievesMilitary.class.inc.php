...
<?php
	...
	if( !class_exists("HumanThievesMilitary")) {
		class HumanThievesMilitary extends MilitaryBase {
			var $picture = "scripts/military/human/thieves.gif";
			var $MilitaryConst = NULL;

			function HumanThievesMilitary( $militaryID ) {

				$this->MilitaryConst = $GLOBALS['MilitaryConst'];

				$info = "A plain attack is not always the best way to go when you are in war with a province. Sometimes i would be wise to use other resources such as thieves to get an adavntage. The thieves will always remember the basic skills of an ordinary fighter and can still be usefull in ordinary attacks. This military unit needs an additional trainingtime of 24 days after finishing recruits training.";
				$type = $this->MilitaryConst->THIEVES;
				$ticks = 24;
				$cost = array("gold"=>800, "metal"=>0, "food"=>80);
