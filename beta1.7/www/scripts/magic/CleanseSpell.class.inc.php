...
class CleanseSpell extends SpellBase {	
	...
	function spellEffect( &$db, $province, $targetProvince, $wizards, $other=false ) 
	{
		...
		$ourWizardsName = strtolower( $milArray['object']->getName() );
			...
			$selectSQL = 	"SELECT S.spellID, S.strength, S.wizards, S.sID, S.casterID
							FROM Spells S 
			...
			if( ( $spells = $db->query( $selectSQL ) ) && $db->numRows() ) {
				...
				if( strlen( $spellsRemovedHtml ) ) {
					...
					/* Soptep: The caster is not informed when his spells are cleansed
						- The spells that are cleansed need to be grouped by the caster
						- Only the casters that theirs spells got cleansed should be informed
					*/
					/*require_once($GLOBALS['path_www_scripts']."Province.class.inc.php");
					$casterProvince = new Province($targetID, $this->dbRef);*/
