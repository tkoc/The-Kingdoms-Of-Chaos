...
function templateDisplay($province, $htmlBody , $cornerpic="../img/space.gif", $left="",$forum=false ) {
	...
	{
		$cornerpic="../img/space.gif";
		$left="";
		 
		if ( ($province->vacationmode==true) && ($province->vacationTicks >= 48))
		{
			if (isset($_POST['Cancel'])) 
				$GLOBALS['database']->query("UPDATE Province set vacationMode='false' where pID='". $province->pID ."'");
			else {
				$htmlBody = "<center>You can cancel vacation mode by pressing the cancel button. Your will be able to use your province as normal.<br /><br />";
				$htmlBody .= "<form action='".$_SERVER['PHP_SELF']."' method='post'>";
				$htmlBody .= "<input type='submit' name='Cancel' value='Cancel Vacation'>";
				$htmlBody .= "</form></center>";
			}
		}
		else {
			$vacationRemainingTicks = 48 - $province->vacationTicks;
			$htmlBody = "<center>Your province is now on vacation mode. You will be able to return to normal mode in $vacationRemainingTicks tick".($vacationRemainingTicks == 1 ? "" : "s").".</center>";
		}
	} // if
	...
	switch ($url) {
		...
		case "Explore.php":
			$page = "Explore";
			break;
		case "provincesRanking.php":
			$page = "Provinces Ranking";
			break;
		case "kingdomsRanking.php":
			$page = "Kingdoms Ranking";
			break;
...
<div class="page-container">
			...
			<div class="header-data">
				<?php if (isset($_SERVER['HTTP_USER_AGENT']) && (strpos($_SERVER['HTTP_USER_AGENT'], 'Opera Mobi') === false)) {?>
							<div class="donation-notify">
								<img src="<?php echo $GLOBALS['currentGameDomain']; ?>images/arrow.png" alt="Arrow">
								<span>&nbsp;TKOC is a free game and it will remain free. If you like it, please donate and help us cover the expenses and keep us motivated :). Visit: <a href="<?php echo $GLOBALS['rootDomain']; ?>worldforum/index.php?action=paypal">I want to donate and contribute.</a></span>
							</div>
				<?php } ?> 
		...
		<div class="content">
			<?php 
			// The maintenances variable should be moved to database
			$maintenance = false; // If you want to put game into maintenance set this as true, only admins will be able to access
			$maintenanceMsg = "The game is under maintenance: Database is being updated to support utf-8 and a security hole is being fixed. It will be back in less than 3-5 minutes.";
			...
			else // Echo here the message that players will see during maintenance
				echo $maintenanceMsg;
			?>
		</div> <!--content-->
