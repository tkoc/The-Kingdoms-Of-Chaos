...
require_once("MilitaryConstants.class.inc.php");
	if( !class_exists("GiantAttackersMilitary")) {
		class GiantAttackersMilitary extends MilitaryBase {
			var $picture = "recruite";
			var $MilitaryConst = NULL;
			
			function GiantAttackersMilitary( $militaryID ) {
				$this->MilitaryConst = $GLOBALS['MilitaryConst'];
				$ticks = 20;
				$info = "To attack others, Fire Giants are the right hotheads for the job. They require a longer training of an additional $ticks days, but this makes them fearsome opponents.";
				$type = $this->MilitaryConst->OFF_SOLDIERS;
				$cost = array("gold"=>700, "metal"=>0, "food"=>0);
				$strength = array("attack"=>4, "defense"=>2);
				$milConst = $GLOBALS['MilitaryConst'];
				$this->MilitaryBase($militaryID, "Fire Giants", $info ,$type, $ticks, $cost, $strength);
			}
